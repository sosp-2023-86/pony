...
namespace dory::pony::tail_p2p::internal {
...
class SyncSender : public Lazy {
 ...
 public:
  ...
  inline static size_t constexpr slotSize(size_t const max_msg_size) {
    ...
    // atomic, and 64-byte aligned to avoid false sharing.
    return (unaligned_size + 64 - 1) & static_cast<size_t>(-64);
  }
    ...
    if (unlikely(outstanding_writes != 0)) {
      ...
      for (auto const &wc : wcs) {
        if (wc.status != IBV_WC_SUCCESS) {
          // TODO: consider the guy as being dead or, for stubborness,
          // re-post the WRITE.
          throw std::runtime_error(
