...
class Receiver {
 ...
 public:
  struct Message {
    Header const *header;
    size_t size;
    uint8_t const *msg() const {
      return reinterpret_cast<uint8_t const *>(header + 1);
    }
    // TODO: to verify that a msg is STILL valid (i.e., hasn't been overwritten)
    // bool valid() const;
  };
  ...
  std::optional<Message> poll() {
    for (size_t i = 0; i < tail; i++) {
      auto const poll_result = tryPoll();
      if (unlikely(std::holds_alternative<Polled>(poll_result))) {
        return std::get<Polled>(poll_result).msg;
      }
 ...
 private:
  ...
  TryPollResult tryPoll() {
    ...
    if (scanning == best_to_deliver ||
      ...
      if (hash ==
          XXH3_64bits(const_cast<uint8_t const *>(data_beginning), size)) {
        ...
        return Polled{const_cast<Header const *>(header), size};
      }
