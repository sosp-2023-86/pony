...
static inline void blake3_final_there(Blake3Hasher &state, uint8_t *const out,
                                      size_t const len) {
  blake3_hasher_finalize(&state, out, len);
}

static inline Blake3Hash blake3_final(Blake3Hasher &state) {
  Blake3Hash ret;
  blake3_final_there(state, ret.data(), Blake3HashLength);
  return ret;
}
